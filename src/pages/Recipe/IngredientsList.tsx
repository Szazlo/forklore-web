 ...
 */
export function IngredientsList({ ingredients }: { ingredients: RecipeData["ingredients"] }) {
	...
	function tickIngredient(checked: boolean, id: string) {
		setStatefulIngredients(prevState => prevState.map(ingredient => (
			{ ...ingredient, checked: ingredient.id === id ? checked : ingredient.checked }
		)));
	}
		...
		<FormGroup>
			{statefulIngredients.map(ingredient =>
				<FormControlLabel
					...
					label={<Typography sx={{ textDecoration: ingredient.checked ? "line-through" : "" }}
														 color={ingredient.checked ? "text.dark" : ""}>{ingredient.name}</Typography>}
				/>,
			)}
		</FormGroup>
	);
