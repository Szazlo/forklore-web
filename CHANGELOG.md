# Project change log

===================

### 0.0.10 2024-09-08

- Added mockup for Recipe Page, which includes
  - Title
  - Publisher
  - Date published
  - Reviews
  - Image
  - Prep time, cooking time and serve count
  - About section
  - Ingredients with checkboxes
  - Steps with checkable boxes
  - Review form
  - "You might like" recipe recommendations
  - Nutrition facts
  - Newsletter subscription form
  - Tags
  - Bookmark, share and print button
- This design is responsive for all screen sizes, however the mobile version needs the action buttons

### 0.0.9 2024-13-07

- Added stock images for the sign in and sign up page for desktop
- Added a box shadow container for the stock image and form
- Added Snackbar Context provider and hook

### 0.0.8 2024-17-05

- Improved design of register and sign in page
- Added basic design of home page that includes recipes
- Fixed issues with the website not working correctly on firebase hosting
- Restructured frontend code for code scalability
- Addes sample recipe data (for testing)

### 0.0.8 2024-05-06

- Added profile button on navbar when user is signed in
- Added (working) sign out button in navbar profile menu
- Made it so every page except the register and login will show the header and footer.

### 0.0.7 2024-05-03

- Added `/recipe` route template.

### 0.06 2024-05-02

- Added animation on scroll for navigation bar
- Added footer
- Added 404 page

### 0.0.5 2024-05-01

- Added navigation bar (for desktop view)

### 0.0.4 2024-05-01

- Added Sign in with Google
- Added Auth store which stores current user

### 0.0.3 2024-05-01

- Register page layout finished
- no functionality yet

### 0.0.2 2024-04-29

- Set up firebase, Shadcn UI and other dependencies

### 0.0.1 2024-04-27

- Initial setup of the project
